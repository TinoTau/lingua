//! CoreEngine 集成测试：测试文本→语音流程（使用 Piper HTTP TTS）
//! 
//! 使用方法：
...
//!   2. 服务正在运行
//! 
//! 注意：此测试只测试 TTS 部分，不涉及完整的 S2S 流程
...
use core_engine::nmt_incremental::{NmtIncremental, TranslationRequest, TranslationResponse};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== CoreEngine Piper TTS 集成测试 ===\n");
    ...
    let chunk = engine.tts().synthesize(request).await
    ...
    let output_file = project_root.join("test_output").join("coreengine_zh_test.wav");
    ...
    println!("  2. 如果正常，可以继续测试完整的 S2S 流程（Whisper → NMT → Piper TTS）");
