...
//!   3. Marian NMT 模型已下载到 core/engine/models/nmt/marian-zh-en/ 或 marian-en-zh/
//!   4. 输入音频文件（WAV 格式，16kHz 单声道）
...
use core_engine::types::{AudioFrame, PartialTranscript};
use std::sync::Arc;
use std::fs;
...
impl VoiceActivityDetector for StubVad {
    async fn detect(&self, frame: AudioFrame) -> core_engine::error::EngineResult<core_engine::vad::DetectionOutcome> {
        Ok(core_engine::vad::DetectionOutcome {
            is_boundary: true,  // 标记为边界，触发 ASR 推理
            confidence: 1.0,
            frame,
        })
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== 完整 S2S 流集成测试（真实 ASR + NMT + Piper TTS） ===\n");

    // 解析命令行参数
    let args: Vec<String> = env::args().collect();
    if args.len() < 2 {
        eprintln!("用法: cargo run --example test_s2s_full_real -- <input_wav_file>");
        eprintln!("示例: cargo run --example test_s2s_full_real -- test_input/chinese_audio.wav");
    ...
    // 构建 CoreEngine
    println!("\n[3/7] 构建 CoreEngine（真实 ASR + NMT + Piper TTS）...");
    
    // 加载中文到英文的 NMT 模型
    let crate_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let nmt_model_dir = crate_root.join("models/nmt/marian-zh-en");
    if !nmt_model_dir.exists() {
        return Err(format!("Marian NMT 模型目录不存在: {}. 请确保模型已导出。", nmt_model_dir.display()).into());
    }
    
    let nmt_impl = core_engine::MarianNmtOnnx::new_from_dir(&nmt_model_dir)
        .map_err(|e| format!("Failed to load MarianNmtOnnx: {}", e))?;
    
    // 保存 ASR 和 NMT 的引用，以便后续直接使用
    let asr_impl = core_engine::asr_whisper::WhisperAsrStreaming::new_from_dir(
        &crate_root.join("models/asr/whisper-base")
    ).map_err(|e| format!("Failed to load Whisper ASR: {}", e))?;
    
    let asr_ref = Arc::new(asr_impl);
    let nmt_ref = Arc::new(nmt_impl);
    ...
    println!("[OK] CoreEngine 初始化成功（ASR 和 NMT 已自动初始化）");
    ...
    for (i, frame) in audio_frames.iter().enumerate() {
        ...
        let asr_result = AsrStreaming::infer(&*asr_ref, asr_request).await
    ...
    let translation_response = NmtIncremental::translate(&*nmt_ref, translation_request).await
    ...
    // 创建 TTS 客户端（直接创建，不通过 CoreEngine）
    ...
    let output_file = project_root.join("test_output").join("s2s_full_real_test.wav");
    ...
    // 清理
    AsrStreaming::finalize(&*asr_ref).await?;
    NmtIncremental::finalize(&*nmt_ref).await?;
