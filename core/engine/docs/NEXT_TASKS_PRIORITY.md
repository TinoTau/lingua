...
#### 1. NMT KV Cache 优化
- **任务 ID**: `nmt-2`
- **状态**: ⚠️ 当前使用 workaround 模式（性能较慢）
- **文件**: `core/engine/src/nmt_incremental/mod.rs`
- **任务**:
  - [ ] 修复 KV cache 的 Reshape 错误
  - [ ] 实现正确的 KV cache 传递（黑盒 Value 方式）
  - [ ] 性能测试和验证
- **预计时间**: 2-3 天
- **优先级原因**: 影响 NMT 性能，当前 workaround 模式较慢
...
1. **NMT KV Cache 优化**是当前最优先的任务，因为它影响性能
