# Sherpa-ONNX-VITS-ZH-LL 模型问题总结

**日期**: 2024-12-19  
**状态**: 阻塞 - 生成的音频无法识别任何一个字  
**优先级**: 高
...
我们测试了 `csukuangfj/sherpa-onnx-vits-zh-ll` 模型（中文 TTS）。虽然模型可以正常加载和推理，生成的音频文件也存在且长度合理，但**所有生成的音频都无法识别任何一个字**，无论使用哪种编码格式或参数组合。
...
- ✅ 推理成功，无错误
- ✅ 生成的音频文件存在且长度合理
- ✅ 语速可以通过 `length_scale` 参数调整
- ❌ **所有生成的音频都无法识别任何一个字**

### 测试文本
- 测试了多个句子（"你好，世界。"、"这是一个测试。" 等）
- 测试了 4 种不同的编码格式
- 测试了 6 组不同的参数组合
- **所有测试结果都无法识别任何一个字**
...
**测试内容**:
- ✅ 测试了 4 种不同的编码格式（直接转换、开头加静音、每个音素后加空白、字之间加空白）
- ✅ 测试了 6 组不同的参数组合（默认、慢速、更慢、不同说话人、低噪声）
- ✅ 测试了不同的空白 token 处理方式（'_' 和 ' '）

**测试结果**:
- ❌ 所有格式和参数组合都无法生成可识别的音频
- ✅ 语速可以通过 `length_scale` 参数调整（1.0-2.0）
- ❌ 但即使语速合理，仍然无法识别任何一个字
...
### 1. 编码格式问题（最可能）
- **假设**: 虽然我们按照 lexicon 转换了，但编码格式可能不对
...
### 2. 参数调优
- ✅ 测试了多组参数组合（noise_scale, length_scale, noise_scale_w, sid）
- ✅ 找到了语速合理的参数组合
- ❌ 但即使语速合理，仍然无法识别任何一个字

### 3. 空白 token 处理
- ✅ 测试了不同的空白 token 使用方式（'_' 和 ' '）
- ✅ 测试了空白 token 的不同插入位置
- ❌ 所有方式都无法生成可识别的音频
...
**行动**:
1. 查找 `csukuangfj/sherpa-onnx-vits-zh-ll` 的 GitHub 仓库或文档
2. 查找 `sherpa-onnx` 的官方文档或示例代码
3. 查看是否有使用示例或文档
4. 确认正确的编码方式和使用方法
5. 修复我们的实现
...
- 模型来源: https://huggingface.co/csukuangfj/sherpa-onnx-vits-zh-ll
- 训练方法: https://github.com/Plachtaa/VITS-fast-fine-tuning
